...
import type { CreatePostInput, UpdatePostParams, UpdatePostInput } from "./post.types";
...
export class PostController {
  ...
  updatePost = catchAsync(
    async (req: Request<UpdatePostParams, {}, UpdatePostInput>, res: Response) => {
      const post = await this.service.updatePost(req.params.postId, req.user!.id, req.body);
      res.status(200).json({ data: post });
    }
  );

  deletePost = catchAsync(
    async (req: Request<UpdatePostParams>, res: Response) => {
